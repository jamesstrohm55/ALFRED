...
from memory.memory_manager import remember, recall, forget, list_memory
import os
...
def get_response(text):
    ...
    try:
        ...
        print(f"Service Commands failed: {e}")
        return "Sorry, I couldn't process your request with the service commands."


def handle_service_commands(text):
    if any(keyword in text.lower() for keyword in ["calendar", "schedule", "remind", "event"]):
        return handle_calendar_command(text)

    if any(keyword in text.lower() for keyword in ["weather", "forecast", "temperature"]):
        return handle_weather_command(text)

    if any(keyword in text.lower() for keyword in ["file", "document", "upload", "download"]):
        return handle_file_command(text)

    if any(keyword in text.lower() for keyword in ["system", "monitor", "status"]):
