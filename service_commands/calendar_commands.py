...
def handle_calendar_command(user_input: str) -> str:
    ...
    if not DATEPARSER_AVAILABLE:
        return (
            "I need the 'dateparser' library to handle calendar commands. "
    ...
    if not parsed.get("title"):
        return (
            "I need more information to add this event. Please try a command like:\n"
            '"Add meeting Team standup tomorrow at 10am for 1 hour"\n'
    ...
    if not parsed.get("start_datetime"):
        return (
            f"I understood you want to add '{parsed['title']}', but I couldn't determine when. "
    ...
    try:
        ...
        return f"'{title}' has been added to your calendar for {start.strftime('%A, %B %d at %I:%M %p')}."
...
def parse_calendar_command(text: str) -> dict:
    ...
    if datetime_text and DATEPARSER_AVAILABLE:
        parsed_dt = dateparser.parse(
            datetime_text, settings={"PREFER_DATES_FROM": "future", "PREFER_DAY_OF_MONTH": "first"}
        )
        if parsed_dt:
